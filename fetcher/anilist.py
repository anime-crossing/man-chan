...
class AnilistApi:
    ...
    @classmethod
    def fetch_search_results(cls, media_type: str, query: str, format: Optional[str] = None):
        variables = {"type": media_type, "search": query, "page": 1, "perPage": 10}
        if format is not None:
            variables["format"] = format

        json_response: Dict[str, Dict] = requests.post(
            AnilistApi.api_url, json={"query": AnilistQueries.search, "variables": variables}
        ).json()
    ...
    def fetch_media_info(cls, media_id: str):
        ...
        json_response = requests.post(
            AnilistApi.api_url, json={"query": AnilistQueries.media, "variables": variables}
        ).json()
    ...
    def fetch_user_media_score(cls, media_id: str, anilist_id: int):
        ...
        json_response = requests.post(
            AnilistApi.api_url, json={"query": AnilistQueries.score, "variables": variables}
        ).json()
...
class _ResultFactory:
    ...
    def media_result(cls, data: Dict[str, Any]) -> '_MediaResult':
        ...
        return _MediaResult(
            ...
            studios = [studio.get("name", "") for studio in data.get("studios", {}).get("nodes", [])],
    ...
    @classmethod
    def staff_list(cls, staff_json: Dict[str, List[Dict[str, Any]]]) -> List['_StaffMember']:
        '''Api result structural model for a list of staff members'''
        result = []

        for edge, node in zip(staff_json.get("edges", []), staff_json.get("nodes", [])):
            result.append(_StaffMember(role=edge.get("role", ""), name=node.get("name", {}).get("full", "")))
