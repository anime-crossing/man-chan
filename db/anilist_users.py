...
class AnilistUsers(Base):
    ...
    def anime_leaderboard(cls) -> List["AnilistUsers"]:
        return cls._query().filter(AnilistUsers.anilist_id.is_not(0)).order_by(AnilistUsers.minutes_watched.desc()).all()  # type: ignore

    @classmethod
    def manga_leaderboard(cls) -> List["AnilistUsers"]:
        return cls._query().filter(AnilistUsers.anilist_id.is_not(0)).order_by(AnilistUsers.chapters_read.desc()).all()  # type: ignore
