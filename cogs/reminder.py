...
class ReminderCog(CommandBase):
    ...
    def load_reminders(self):
        try:
            with open(self.REMINDERS_FILE, "r") as f:
                ...
                self.reminders = {
                    k: v for k, v in self.reminders.items() if not v.get("sent", False)
                }
        ...
        for reminder_id, reminder in self.reminders.items():
            ...
            if current_time >= reminder_time:
                try:
                    ...
                    if not channel:
                        logger.error(
                            f"Could not find channel with ID {reminder['channel_id']}"
                        )
                        continue

                    if not user:
                        logger.error(
                            f"Could not find user with ID {reminder['user_id']}"
                        )
                        continue

                    if not set_by:
                        logger.error(
                            f"Could not find user who set reminder with ID {reminder['set_by']}"
                        )
                        continue
                    ...
                    if not channel.permissions_for(channel.guild.me).send_messages:
                        logger.error(
                            f"Bot does not have permission to send messages in channel {channel.name}"
                        )
                        continue
                    ...
                    if user.id == reminder["set_by"]:
                        ...
                        await channel.send(
                            f"🔔 {user.mention} Reminder from {set_by.mention}: {reminder['message']}"
                        )
    ...
    async def set_reminder(self, ctx: commands.Context, *, reminder_text: str):
        ...
        try:
            ...
            if words[0].startswith("<@") and words[0].endswith(">"):
                ...
                if not target_user:
                    await ctx.send(
                        "I couldn't find that user. Please make sure to mention a valid user."
                    )
                    return
            ...
            if not reminder_time:
                await ctx.send(
                    "I couldn't understand the time format. Try something like 'in 2 hours' or 'tomorrow at 3pm'"
                )
                return
            ...
            await ctx.send(
                f"Reminder set! I'll remind {target} about '{message}' at {reminder_time.strftime('%Y-%m-%d %H:%M:%S')}"
            )
    ...
    async def list_reminders(self, ctx: commands.Context):
        ...
        user_reminders = [
            r for r in self.reminders.values() if r["user_id"] == ctx.author.id
        ]
